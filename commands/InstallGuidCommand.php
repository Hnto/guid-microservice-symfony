...
class InstallGuidCommand extends BaseCommand
{
    ...
    protected function configure()
    {
        $this
            ->setName('install')
            ->setDescription('Installs the guid generator micro service applications')
    ...
    private function copyExampleEnvToEnv()
    {
        ...
        if (copy($this->basePath . 'src/.env.dist', 'src/.env') === false) {
            $this->removeEnvFiles();
            $this->sendErrorMessage(
                'could not copy env examples to env file, install failed. Please modify the changes manually'
            );
    ...
    private function setupApplication()
    {
        ...
        if ($process->isSuccessful()) {
            ...
            $this->removeEnvFiles();
            $this->sendErrorMessage($process->getOutput());
        }
    }
